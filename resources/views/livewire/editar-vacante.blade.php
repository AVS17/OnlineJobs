<form action="" class="space-y-5 md:w-1/2" wire:submit.prevent='editarVacante'>
    <!-- titulo -->
    <div>
    ...
    <!-- Imagen -->
    <div>
        <x-input-label for="image_new" :value="__('Imagen')" />
        <x-text-input wire:model="image_new" id="image" class="block w-full mt-1" type="file" accept="image/*" />
        <div class="my-5 w-80">
        ...
        <div class="my-5 w-80">
            @if ($image_new)
            Imagen Nueva:
            <img src="{{$image_new->temporaryUrl()}}">
            @endif
        </div>
